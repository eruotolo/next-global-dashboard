'use client';

import { useCallback, useEffect, useState } from 'react';

import { getAllTickets } from '@/actions/Settings/Tickets';
import NewTicketsModal from '@/components/Modal/Setting/Tickets/NewTicketsModal';
import { ticketColumnsNew } from '@/components/Tables/Setting/Ticket/TicketColumnsNew';
import { TanTable } from '@/components/TanTable';
import type { SimpleTicketQuery } from '@/types/settings/Tickets/TicketInterface';

export default function TicketTableNew() {
    const [ticketData, setTicketData] = useState<SimpleTicketQuery[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const fetchTickets = useCallback(async () => {
        setIsLoading(true);
        try {
            const data = await getAllTickets();
            const transformedData = data.map((ticket) => ({
                id: ticket.id,
                code: ticket.code,
                title: ticket.title,
                userName: ticket.userName,
                userLastName: ticket.userLastName,
                status: ticket.status,
                priority: ticket.priority,
            }));
            setTicketData(transformedData);
            setError(null);
        } catch (error) {
            console.error('Error al obtener los tickets:', error);
            setError('Error al obtener los tickets');
        } finally {
            setIsLoading(false);
        }
    }, []);

    useEffect(() => {
        fetchTickets();
    }, [fetchTickets]);

    return (
        <>
            <div className="flex h-auto w-full justify-between">
                <div>
                    <h5 className="mb-[5px] text-[20px] leading-none font-medium tracking-tight">
                        Tickets (Nueva Tabla Refactorizada)
                    </h5>
                    <p className="text-muted-foreground text-[13px]">
                        Implementación con Fábrica de Columnas y Contexto
                    </p>
                </div>
            </div>
            <div className="mt-[20px]">
                {error && <p className="mb-4 text-red-500">{error}</p>}
                <TanTable
                    columns={ticketColumnsNew}
                    data={ticketData}
                    loading={isLoading}
                    filterPlaceholder="Buscar en todos los campos..."
                    toolbarActions={<NewTicketsModal refreshAction={fetchTickets} />}
                    refreshData={fetchTickets}
                />
            </div>
        </>
    );
}
